...
$car_interrior_color_from = 0;
$car_interrior_color_to = 0;
$car_exterior_color = '';
$car_mileage_from = 0;
$car_mileage_to = 0;
$adv_params = array();
...
if (isset($_GET['car_doors_count_from'])) {
	$car_doors_count_from = $_GET['car_doors_count_from'];
}

if (isset($_GET['car_doors_count_to'])) {
    $car_doors_count_to = $_GET['car_doors_count_to'];
}

if (isset($_GET['car_interrior_color_from'])) {
	$car_interrior_color_from = $_GET['car_interrior_color_from'];
}

if (isset($_GET['car_interrior_color_to'])) {
    $car_interrior_color_to = $_GET['car_interrior_color_to'];
}
						...
						<div class="row">
							<div class="col-xs-6">
								<p>
									...
									<?php
									$now = (int) date("Y") + 1;
									$years = array('any' => __("None", cardealer));
									for ($i = $now; $i >= 1900; $i--) {
										$years[$i] = $i;
									}
									?>
                    ...
                    <div class="row">
                        <div class="col-xs-6">
                            ...
                            <?php if (!empty($show_doors_count)) { ?>
                                <p>
                                    <label for="tmm_qs_doors_<?php echo $uniqid; ?>"><?php _e("Door Count", TMM_CC_TEXTDOMAIN) ?><span> <?php _e("min", TMM_CC_TEXTDOMAIN) ?></span>:</label>
                                    <select id="tmm_qs_doors_<?php echo $uniqid; ?>" name="car_doors_count_from">
                                        <option value="0"><?php _e('None', 'cardealer'); ?></option>
                                        ...
                                        <?php endfor; ?>
                                        <?php for ($i = 21; $i <= 40; $i++): ?>
                                            <?php array_push($height_lift,$i*500); ?>
                                        <?php endfor; ?>
                                        <?php for ($i = 21; $i <= 30; $i++): ?>
                                            <?php array_push($height_lift,$i*1000); ?>
                                        <?php endfor; ?>
                                        <?php array_push($height_lift,35000,40000); ?>
                                        <?php for ($i = 0; $i < sizeof($height_lift); $i++): ?>
                                            <option <?php selected($car_doors_count_from, $height_lift[$i]); ?> value="<?php echo $height_lift[$i] ?>"><?php echo $height_lift[$i] ?></option>
                                        <?php endfor; ?>
                                    </select>
                                </p>
                            <?php } ?>
                        ...
                        <div class="col-xs-6">
                            ...
                            <?php if (!empty($show_doors_count)) { ?>
                                <p>
                                    <label for="tmm_qs_doors_<?php echo $uniqid; ?>"><?php _e("Door Count", TMM_CC_TEXTDOMAIN) ?><span> <?php _e("max", TMM_CC_TEXTDOMAIN) ?></span>:</label>
                                    <select id="tmm_qs_doors_<?php echo $uniqid; ?>" name="car_doors_count_to">
                                        <option value="0"><?php _e('None', 'cardealer'); ?></option>
                                        <?php $height_lift = array(); ?>
                                        <?php for ($i = 20; $i <= 100; $i++): ?>
                                            <?php array_push($height_lift,$i*100); ?>
                                        ...
                                        <?php array_push($height_lift,35000,40000); ?>
                                        <?php for ($i = 0; $i < sizeof($height_lift); $i++): ?>
                                            <option <?php selected($car_doors_count_to, $height_lift[$i]); ?> value="<?php echo $height_lift[$i] ?>"><?php echo $height_lift[$i] ?></option>
                                        <?php endfor; ?>
                                    </select>
                                </p>
                            <?php } ?>
                ...
                <fieldset>
					...
					<div class="row">
						<div class="col-xs-6">
                            <!-- Interior Color from-->
							<p>
								<label for="tmm_qs_interrior_color_<?php echo $uniqid; ?>"><?php _e("Interior Color", TMM_CC_TEXTDOMAIN) ?><span> <?php _e("min", TMM_CC_TEXTDOMAIN) ?></span>:</label>
								<select id="tmm_qs_interrior_color_<?php echo $uniqid; ?>" name="car_interrior_color_from">
									<option value="0"><?php _e("None", cardealer) ?></option>
									<?php if (!empty($car_int_colors)) { ?>
										<?php foreach ($car_int_colors as $color => $color_name) { ?>
											<option <?php selected($car_interrior_color_from, $color); ?> value="<?php echo $color ?>"><?php _e($color_name, TMM_CC_TEXTDOMAIN); ?></option>
										<?php } ?>
									<?php } ?>
								</select>
							</p>
						...
						<div class="col-xs-6">
                            <!-- Interior Color to-->
                            <p>
                                <label for="tmm_qs_interrior_color_<?php echo $uniqid; ?>"><?php _e("Interior Color", TMM_CC_TEXTDOMAIN) ?><span> <?php _e("max", TMM_CC_TEXTDOMAIN) ?></span>:</label>
                                <select id="tmm_qs_interrior_color_<?php echo $uniqid; ?>" name="car_interrior_color_to">
                                    <option value="0"><?php _e("None", cardealer) ?></option>
                                    <?php if (!empty($car_int_colors)) { ?>
                                        <?php foreach ($car_int_colors as $color => $color_name) { ?>
                                            <option <?php selected($car_interrior_color_to, $color); ?> value="<?php echo $color ?>"><?php _e($color_name, TMM_CC_TEXTDOMAIN); ?></option>
                                        <?php } ?>
                                    <?php } ?>
                                </select>
                            </p>
